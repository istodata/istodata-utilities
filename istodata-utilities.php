...
define('IU_PLUGIN_VERSION', '1.8.3');
...
function iu_settings_page() {
    ...
    if (isset($_GET['clear_update_cache']) && $_GET['clear_update_cache'] == '1' && wp_verify_nonce($_GET['_wpnonce'], 'iu_clear_cache')) {
        $cache_key = 'iu_remote_version_' . md5(IU_UPDATE_URL);
...
// Plugin update constants
define('IU_UPDATE_URL', 'https://portal.istodata.com/repo/istodata-utilities/update.json');
...
function iu_get_remote_version() {
    ...
    // Fetch remote data
    $request = wp_remote_get(IU_UPDATE_URL, array(
        'timeout' => 15,
        'sslverify' => false,
...
// Improve HTTP request reliability for our plugin updates
function iu_http_request_args($args, $url) {
    // Only modify requests to our update server
    if (strpos($url, 'portal.istodata.com') !== false) {
        $args['timeout'] = 60; // Increase timeout
        $args['sslverify'] = false; // Skip SSL verification if needed
